...
export function parseBibTeX(bibtexContent: string, locale?: string): Publication[] {
  ...
  return entries.map((entry: { entryType: string; citationKey: string; entryTags: Record<string, string> }, index: number) => {
    ...
    const publication: Publication = {
      ...
      selected,
      preview,
      displayBibtex: tags.display_bibtex,

      // Store original BibTeX (excluding custom fields)
      bibtex: reconstructBibTeX(entry, ['selected', 'preview', 'description', 'keywords', 'code', 'display_bibtex']),
    };
...
function parseAuthors(authorsStr: string, highlightNames: string[]): Array<{ name: string; isHighlighted?: boolean; isCorresponding?: boolean; isCoAuthor?: boolean }> {
  ...
  return authorsStr
    ...
    .map(author => {
      ...
      // Check for equal-contribution and corresponding author markers.
      const isCoAuthor = name.includes('*') || name.includes('#');
      const isCorresponding = name.includes('†') || /\\(?:dagger|textdagger)\b/.test(name);

      // Remove special markers from name before display and highlight matching.
      name = name.replace(/[*#†]/g, '').replace(/\\(?:dagger|textdagger)\b/g, '');
